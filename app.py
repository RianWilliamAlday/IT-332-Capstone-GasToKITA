import subprocess, threading, time, sys, os, json, flet as ft
...
def is_backend_running():
    try:
        with socket.create_connection(("127.0.0.1", 8000), timeout=1): return True
    except: return False
...
def run_backend():
    ...
    for line in proc.stdout: print("[backend]", line.strip())
...
async def main(page: ft.Page):
    ...
    loading = ft.Container(expand=True, alignment=ft.Alignment.CENTER, content=ft.Text("Starting backend...", size=18))
    page.add(loading)
    page.update()
    ...
    try:
        token = await page.shared_preferences.get("gastokita.auth_token")
        if token:
            async with httpx.AsyncClient(timeout=5.0) as client:
                resp = await client.get(f"{API_URL}/api/auth/me", headers={"Authorization": f"Bearer {token}"})
                if resp.status_code == 200:
                    AUTH["token"] = token
                    AUTH["user"] = resp.json()
                    AUTH["role"] = resp.json().get("role")
                    page.controls.clear()
    ...
    async def login_employee(e):
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            try:
                resp = await client.post(f"{API_URL}/api/auth/employee/login", json={"email": email_field.value.strip(), "password": password_field.value})
                if resp.status_code != 200:
                    status_text.value = f"Login failed"
                    page.update()
                    return
                data = resp.json()
                ...
                me = await client.get(f"{API_URL}/api/auth/me", headers={"Authorization": f"Bearer {token}"})
                user_data = me.json() if me.status_code == 200 else {}

                AUTH["token"] = token
                AUTH["user"] = user_data
                AUTH["role"] = user_data.get("role", "EMPLOYEE")
                await page.shared_preferences.set("gastokita.auth_token", token)
                await page.shared_preferences.set("gastokita.user_json", json.dumps(user_data))
                print("[LOGIN] Saved token to shared_preferences")
